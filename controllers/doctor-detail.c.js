...
exports.viewDetail = async (req, res, next) => {
    try {
        ...
        if (req.session.Username) {
            res.render('./doctor/detailDoctor', { data: rs[0], display1: "d-none", display2: "d-block", role:role, username:req.session.Username});
        }
        else {
            res.render('./doctor/detailDoctor', { data: rs[0], display1: "d-block", display2: "d-none", role:role});
        }
    } catch (err) {
        next(err);
    }
