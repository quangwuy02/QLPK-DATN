...
exports.render = async (req, res, next) => {
    try {
        res.render('./auth/login', { errWrongPassword: "none", errWrongUsername: "none", display1: "d-block", display2: "d-none", role: "patient" });
    } catch (err) {
        next(err);
    }
}
    ...
    try {
        ...
        if (user.role == "patient") {
            userM.getByUsername(user.Username).then(rs => {
                if (rs.length == 0) {
                    res.render('./auth/login', { errWrongPassword: "none", errWrongUsername: "block", Username: user.Username, Password: user.Password, display1: "d-block", display2: "d-none", role: user.role });
                    return false;
                }
                else {
                    ...
                    if (pwDb !== (pwHashed + salt)) {
                        res.render('./auth/login', { errWrongPassword: "block", errWrongUsername: "none", Username: user.Username, Password: user.Password, display1: "d-block", display2: "d-none", role: user.role });
                        return false;
                    }
