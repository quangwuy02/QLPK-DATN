...
exports.render= async(req, res, next) =>{
    try {
        res.render('./auth/register', {display:"d-none",display1:"d-block",display2:"d-none"});
    } catch(err) {
        next(err);
    }
}
    ...
    try {
        ...
        userM.getByUsername(user.Username).then(rs=>{
            if (rs.length==0) {
                ...
                user.Password=pw;
                res.render('./auth/register',{user:user, display: "block",display1:"d-block",display2:"d-none", role:"patient"});
            }
        })
    } catch(err) {
        next(err);
